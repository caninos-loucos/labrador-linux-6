...
static int dwc3_caninos_probe(struct platform_device *pdev)
{
	...
	simple->enable_gpio = devm_gpiod_get_optional(dev, "enable", 0);
	if(simple->enable_gpio == NULL)
		dev_warn(dev, "not using enable gpio");
	else
		gpiod_direction_output(simple->enable_gpio, 1);
	...
	if (ret)
	{
		...
		if (simple->enable_gpio)
			gpiod_set_value(simple->enable_gpio, 0);	
