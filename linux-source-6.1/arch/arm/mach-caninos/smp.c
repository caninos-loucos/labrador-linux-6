...
static void caninos_k5_smp_secondary_init(unsigned int cpu)
{
	trace_hardirqs_off();
	write_pen_release(-1);
...
static inline bool sps_power_check(u32 ackbit)
{
	return !!(readl(sps_base_addr) & BIT(ackbit));
}

static inline bool sps_power_on(u32 pwrbit, u32 ackbit)
{
...
static inline bool cpu_power_on(unsigned int cpu)
{
...
static int caninos_k5_smp_boot_secondary(unsigned int cpu,
